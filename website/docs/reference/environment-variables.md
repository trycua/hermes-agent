...
| `SLACK_ALLOWED_CHANNELS` | Comma-separated Slack channel IDs. When set, a non-DM conversation must match an exact ID or an allowed private-channel prefix. |
| `SLACK_ALLOWED_PRIVATE_CHANNEL_PREFIXES` | Comma-separated channel-name prefixes. Matches only private channels where Slack confirms the bot is a member; failures deny access. |
